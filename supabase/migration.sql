...
CREATE POLICY "Feed posts visible to friends" ON feed_posts FOR SELECT USING (TRUE);
...
INSERT INTO shop_items (key, name, description, type, coin_cost, rarity, emoji) VALUES
  ...
  -- Head cosmetics
  ('crown','Coroa Real','Um símbolo de poder','head',300,'legendary','👑'),
  ('wizard_hat','Chapéu de Mago','Conjure seu potencial','head',150,'epic','🎩'),
  ('helmet','Capacete de Batalha','Proteção na jornada','head',80,'rare','⛑️'),
  ('laurel','Coroa de Louros','Para os vitoriosos','head',200,'epic','🌿'),
  -- Face cosmetics
  ('sunglasses','Óculos Escuros','Para os descolados','face',60,'common','😎'),
  ('monocle','Monóculo','Sofisticação pura','face',100,'rare','🧐'),
  ('mask','Máscara Mística','Guardador de segredos','face',180,'epic','🎭'),
  -- Body cosmetics
  ('armor','Armadura Épica','Forjada nas profundezas','body',250,'epic','🥋'),
  ('robe','Manto do Sábio','Sabedoria ancestral','body',200,'rare','🧥'),
  ('cape','Capa do Herói','Para os lendários','body',350,'legendary','🦸'),
  -- Accessories
  ('sword','Espada Lendária','Cortando objetivos','accessory',300,'legendary','⚔️'),
  ('crystal','Orbe de Cristal','Energia mística','accessory',180,'epic','🔮'),
  ('scroll','Pergaminho Sagrado','Conhecimento infinito','accessory',120,'rare','📜'),
  ('shield_acc','Escudo de Batalha','Defesa suprema','accessory',200,'epic','🛡️')
...
INSERT INTO badges (key, title, description, icon, requirement_type, requirement_value) VALUES
  ...
  ('diamond_streak','💎 Diamond Streak','50 dias de streak','💎','max_streak',50),
  ...
  ('hydration_hero','💧 Hydration Hero','Meta de água por 30 dias','💧','water_streak',30),
...
INSERT INTO challenge_pool (key, title, description, type, target_value, reward_coins, icon) VALUES
  ...
  ('events_week','Zero Falhas','Complete todos os eventos do calendário','calendar_completion',100,150,'📅'),
  ('xp_200','Ganho de XP','Ganhe 200 XP em qualquer skill esta semana','xp_earned',200,100,'⚡'),
  ('expense_daily','Registro Diário','Registre ao menos 1 gasto por dia','expense_daily',7,100,'💰'),
  ...
  ('pomodoro_3skills','Polímata','Faça Pomodoros em 3 skills diferentes','skills_pomodoro',3,180,'🎪'),
  ('xp_500','Grind Épico','Ganhe 500 XP esta semana','xp_earned',500,250,'💎')
ON CONFLICT (key) DO NOTHING;

-- ----------------------------------------
-- SEED NEW ACHIEVEMENTS (v2)
-- ----------------------------------------
INSERT INTO achievements (key, title, description, icon, category) VALUES
  ...
  -- Financial new
  ('no_spend_day','🚫 No Spend Day','Um dia sem gastos','🚫','financial'),
  ...
  ('level_up_50','🚀 Relentless','Subir de nível 50 vezes','🚀','milestones'),
